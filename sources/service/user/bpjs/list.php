...
try {
    ...
    if (!empty($filterRules)){
        ...
        $sql .= " FAVID,DISPLAYTEXT FROM ".TABLENAME." where Tipe='B' AND IDUSER='$_iduser' ";
        ...
        $sql .= " ) AS TBL ";
        $sql .= " WHERE NUMBER BETWEEN ((".$_page." - 1) * ".$_rows." + 1) AND (".$_page." * ".$_rows.") ";
        $sql .= " ORDER BY ".$sort." ".$order;
    } else {
        $sql = "SELECT * FROM ( SELECT ROW_NUMBER() OVER(ORDER BY ".$sort.") AS NUMBER, * FROM ".TABLENAME." where Tipe='B' AND IDUSER='$_iduser') AS TBL ".
            " WHERE NUMBER BETWEEN ((".$_page." - 1) * ".$_rows." + 1) AND (".$_page." * ".$_rows.") ".
            " ORDER BY ".$sort." ".$order;
    }
