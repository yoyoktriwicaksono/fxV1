...
$sqlcount="select COUNT(*) as TOTAL from MST_WAREHOUSE";

if (!empty($filterRules)){
    $sql = "SELECT outer.*  FROM (SELECT ROWNUM rn, inner.* FROM ( ";
    $sql .= "select * from MST_WAREHOUSE ";
    ...
    $sql = "SELECT outer.*  FROM (SELECT ROWNUM rn, inner.* ".
        "FROM (select * ". 
        "from MST_WAREHOUSE ORDER BY ".$sort." ".$order." ) inner) outer WHERE outer.rn >= ".$_limit1." AND outer.rn <= ".$_limit2;
}
