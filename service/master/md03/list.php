...
$result = array();  
$sqlcount="select COUNT(*) as TOTAL from INV_UNIT_CONVERSION";

if (!empty($filterRules)){
    $sql = "SELECT outer.*  FROM (SELECT ROWNUM rn, inner.* FROM ( ";
    $sql .= "select * from INV_UNIT_CONVERSION ";
    ...
    //$sql = "select * from UNIT_CONVERSION ORDER BY ".$sort." ".$order;
    $sql = "SELECT outer.*  FROM (SELECT ROWNUM rn, inner.* ".
        "FROM (select * ". 
        "from INV_UNIT_CONVERSION ORDER BY ".$sort." ".$order." ) inner) outer WHERE outer.rn >= ".$_limit1." AND outer.rn <= ".$_limit2;
}
