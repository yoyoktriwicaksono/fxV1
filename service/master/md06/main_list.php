...
$sqlcount="select COUNT(*) as TOTAL from INV_SIZE_GROUP";

if (!empty($filterRules)){
    $sql = "SELECT outer.*  FROM (SELECT ROWNUM rn, inner.* FROM ( ";
    $sql .= "select * from INV_SIZE_GROUP ";
    ...
    $sql = "SELECT outer.*  FROM (SELECT ROWNUM rn, inner.* ".
        "FROM (select * ". 
        "from INV_SIZE_GROUP ORDER BY ".$sort." ".$order." ) inner) outer WHERE outer.rn >= ".$_limit1." AND outer.rn <= ".$_limit2;
}
